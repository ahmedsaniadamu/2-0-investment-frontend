...
export const calculateInvestmentMetrics = (investment: any) => {
  ...
  const averageRoi = () => {
    const roiValue = investment?.plan?.roi?.replace("%", "");
    if (roiValue.includes("-")) {
      const [min, max] = roiValue.split("-").map(Number);
      const avgRoi = (min + max) / 2;
      return Number(avgRoi.toFixed(2));
    } else {
      return Number(parseFloat(roiValue).toFixed(2));
    }
  };

  const roi = averageRoi();
