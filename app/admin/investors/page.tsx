...
import { CircleUserRound, CheckCircle, Clock, XCircle, Search, Filter, DotSquare, EllipsisVertical, MessageSquareQuote } from "lucide-react";
        ...
        <header className='md:flex justify-between items-center'>
          ...
          <div>
            <div className='flex items-center'>
              ...
              <Button
                onClick={() => {
                  push('/admin/investors/feedbacks')
                }}
                className='h-12 text-white bg-primary ml-2' variant="outline">
                <MessageSquareQuote /> Investors Feedback
              </Button>
            </div>
          </div>
        </header>
                    ...
                    <TableBody>
                      {
                        ...
                        investors?.data?.map((investor: any, index: number) => (
                          <TableRow key={investor?.id}>
                            ...
                            <TableCell>
                              <span
                                className={`px-3 py-1 text-sm rounded-full ${investor?.kycRequests?.status === "approved"
