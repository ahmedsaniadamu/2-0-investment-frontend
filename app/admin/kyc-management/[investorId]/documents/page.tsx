  ...
  const handleApprove = async (kyc: any) => {
    ...
    if(ok){
      try {
        ...
        toastMessage("success", "Success", "KYC document approved successfully, Investor will recieve a mail about this rejection");
         ...
         <div className="p-1 space-y-6">
          ...
          <h1 className="text-2xl max-[500px]:mb-3 font-semibold">
            ...
            <div className='md:flex md:gap-4 md:items-center max-[500px]:grid max-[500px]:grid-cols-2 max-[500px]:gap-4'>
                ...
                <Button style={{ opacity: investorKyc?.status === 'rejected' ? 0.5 : 1}} disabled={investorKyc?.status === 'rejected'} onClick={handleReject} className='h-12 text-white ml-2' variant='destructive'>
                    Reject
                </Button>
                ...
                <Button asChild className='h-12 bg-white ml-2' variant='outline'>
                    <Link href={`/admin/kyc-management/${investorKyc?.investor?.id}`}>Back</Link>
                </Button>
                 </div>
             </div>
