...
export const sharedPlans = {
  getPlans: ({ search }: { search?: string }): Promise<any> =>
    client.get(`/plans?limit=10000${search ? `&search=${search}` : ""}`).then(({ data }) => data),
}
...
export const adminPlans = {
  getPlans: ({
    ...
    client.get(`/admin/plans?page=${page}&limit=${limit}${search ? `&search=${search}` : ""
      }`).then(({ data }) => data),
  ...
  getPlansInvestments: ({
    ...
    client.get(`/admin/plans/investments/${id}?page=${page}&limit=${limit}${search ? `&search=${search}` : ""
      }`).then(({ data }) => data),
