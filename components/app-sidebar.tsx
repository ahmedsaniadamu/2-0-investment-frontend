...
export function AppSidebar({ items = investorModules }: {
  items?: {
    title: string
    url: string
    icon: any
            ...
            <SidebarMenu>
              {items.map((item, index: number) => (
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton className={`font-semibold text-white px-3 ${index === 0 && pathname === '/investor' ? "border-r-3 pl-2 border-white rounded-none" :
